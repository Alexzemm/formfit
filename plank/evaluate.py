...
model.load_state_dict(torch.load("plank_transformer.pth", map_location=device))
model.to(device)
model.eval()

classes = ['correct', 'hips_down', 'hips_up']  # Ensure matches training
...
while True:
    ...
    if len(sequence) == seq_len:
        ...
        class_colors = {
            ...
            'hips_down': (0, 0, 255),     # Red
            'hips_up': (255, 0, 0)     # Blue
        }
    ...
    cv2.namedWindow("Plank Pose Classifier", cv2.WINDOW_NORMAL)

    cv2.setWindowProperty("Plank Pose Classifier", cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)

    cv2.imshow("Plank Pose Classifier", frame)
