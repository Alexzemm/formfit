...
model.load_state_dict(torch.load("pushup_transformer.pth", map_location=device))
model.to(device)
model.eval()

classes = ['correct', 'pike', 'snake']  # Ensure matches training
...
while True:
    ...
    cv2.setWindowProperty("Pushup Pose Classifier", cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)

    cv2.imshow("Pushup Pose Classifier", frame)
