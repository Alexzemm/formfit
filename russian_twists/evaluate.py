...
model.load_state_dict(torch.load("russian_transformer.pth", map_location=device))
model.to(device)
model.eval()

classes = ['correct', 'legs_bent']  # Ensure matches training
...
while True:
    ...
    # Un-mirror the camera (flip horizontally)
    ...
    cv2.namedWindow("Russian Twists Pose Classifier", cv2.WINDOW_NORMAL)

    cv2.setWindowProperty("Russian Twists Pose Classifier", cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)

    cv2.imshow("Russian Twists Pose Classifier", frame)
